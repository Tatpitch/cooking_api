...
async def test_get_recipe_detail(async_client: AsyncClient):
    """
        Асинхронный тест для получения детальной информации о рецепте из тестовой БД - GET запрос
        """
...
async def test_add_ingredient(async_client: AsyncClient):
    ...
    response = await async_client.post('/ingredients', json=data_test_add_ingredient )

    assert response.status_code == 200
    assert (response.json()["ingredient_name"]) == "adding ingredient"
    assert (response.json()["ingredient_description"]) == "description for adding ingredient"
...
async def test_add_recipe(async_client: AsyncClient):
    ...
    data_test_add_recipe = {
        ...
        "ingredients": [{"ingredient_id": 1, "quantity": "quantity ingredient  adding recipe"}],
         }
